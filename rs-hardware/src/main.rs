...
use icm42688::{ICM42688, Ready, Uninitialized};
...
where
    I2C: HalI2c + 'static,
{
    match sensor.read_fifo_count().await {
        Ok(count) => defmt::info!("FIFO count: {}", count),
        Err(e) => defmt::error!("Failed to read FIFO count: {:?}", e),
    }
